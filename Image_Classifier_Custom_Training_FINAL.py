"""
Custom Image Classifier — Modern UI

- ResNet-18 backbone is FROZEN (feature extractor only)
- You define classes, provide training images, then classify
- Training required before classification
- Weights and class info persist to disk between sessions
...
class ModernClassifierApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        browse_btn = ttk.Button(row1, text="Browse", command=self._browse_file)
        browse_btn.pack(side=tk.LEFT, padx=(6, 0))
        classify_btn = ttk.Button(row1, text="Classify", style="Accent.TButton", command=self._start_single)
        classify_btn.pack(side=tk.LEFT, padx=(6, 0))
        ...
        choose_btn = ttk.Button(row2, text="Choose...", command=self._choose_folder)
        choose_btn.pack(side=tk.RIGHT, padx=(6, 0))
        ...
        # --- TRAINING SECTION (bottom of right column) ---
        ...
        add_btn = ttk.Button(btn_row, text="+ Add Class", style="Success.TButton", command=self._add_class)
        add_btn.pack(side=tk.LEFT, padx=(0, 4))
        remove_btn = ttk.Button(btn_row, text="Remove Class", style="Danger.TButton", command=self._remove_class)
        remove_btn.pack(side=tk.LEFT)

        train_btn = ttk.Button(train_card, text="Train", style="Accent.TButton", command=self._start_training)
        train_btn.pack(fill=tk.X, pady=(6, 4))
        ...
        # Hover cursors
        for b in (classify_btn, self.classify_folder_btn, browse_btn, choose_btn, add_btn, remove_btn, train_btn):
    ...
    def _set_status(self, t):  self.after(0, lambda: self.status_var.set(t))
